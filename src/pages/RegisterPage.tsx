  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      const data = await api<{ token: string; user: User }>("/auth/register", {
        method: "POST",
        body: JSON.stringify({ fullName: values.full_name, email: values.email, password: values.password }),
      })
      saveToken(data.token)
      await refreshAuth()
      ...
      toast.error("Registrasi gagal: " + (error instanceof Error ? error.message : "Terjadi kesalahan"))
    } finally {
      setLoading(false)
    }
  }
        ...
        <Card className="border-border/60 shadow-xl">
          ...
          <CardContent>
            {import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
              <div id="google-register" className="mb-4 flex justify-center" />
            ) : (
            ...
            <div className="mb-4 flex items-center gap-3 text-xs text-muted-foreground"><div className="h-px flex-1 bg-border" />atau daftar manual<div className="h-px flex-1 bg-border" /></div>
